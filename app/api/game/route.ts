...
async function change(code:string,token:string,fn:(g:Game)=>void){for(let n=0;n<12;n++){const row=await read(code);if(!row)return fail('Kambario nėra arba jis baigė galioti. Patikrink kodą.',404);if(!row.game.players.some(p=>p.token===token))return fail('Šis telefonas neprijungtas prie kambario.',403);expireAnswers(row.game);fn(row.game);const result=await db().prepare('UPDATE games SET state = ?, version = version + 1 WHERE code = ? AND version = ?').bind(JSON.stringify(row.game),code,row.version).run();if(result.meta.changes)return json(publicGame(row.game,token));}return fail('Keli žaidėjai atsakė vienu metu. Bandyk dar kartą.',409);}
...
export async function POST(request:Request){
 ...
 for(let n=0;n<8;n++){const row=await read(code);if(!row)return fail('Kambario nėra. Patikrink kodą.',404);const g=row.game;if(g.players.some(p=>p.token===token))return json(publicGame(g,token));if(g.solo||g.status!=='lobby')return fail('Šiame kambaryje jau žaidžiama.');if(g.players.length>=MAX_PLAYERS)return fail(`Kambarys pilnas (daugiausia ${MAX_PLAYERS} žaidėjai).`);g.players.push({token,name,score:0,answers:{},ready:false});const r=await db().prepare('UPDATE games SET state = ?,version = version + 1 WHERE code = ? AND version = ?').bind(JSON.stringify(g),code,row.version).run();if(r.meta.changes)return json(publicGame(g,token));}return fail('Bandyk prisijungti dar kartą.',409);
 }
 return await change(code,token,g=>{
 const p=g.players.find(p=>p.token===token)!;const q=g.questions[g.index];
 if(b.action==='start'){if(g.players[0].token!==token)throw Error('Žaidimą pradeda kambario kūrėjas.');if(g.status!=='lobby'||g.players.length<2)throw Error('Palauk, kol prisijungs bent vienas varžovas.');g.status='playing';g.startedAt=Date.now();return;}
 ...
 if(b.action==='next'){if(g.status!=='playing'||q?.id!==b.questionId)throw Error('Klausimas jau pasikeitė.');if(!g.players.every(p=>p.answers[q.id]))throw Error('Palauk, kol atsakys visi žaidėjai.');p.ready=true;if(g.players.every(p=>p.ready)||g.players[0].ready){if(g.index===g.questions.length-1)g.status='finished';else{g.index++;g.startedAt=Date.now();for(const p of g.players)p.ready=false;}}return;}
 if(b.action==='retry'){
